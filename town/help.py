...
help_content = "The labyrinth is a 16 level maze of treasure rooms, often filled with treasure, sometimes found with " \
               "traps. Treasure rooms are found at the end of hallways, behind unlocked doors, but to get to the " \
               "next level, you will need a skeleton key. Keys are hidden in chests (along with maps occasionally), " \
               "so if you haven't found the key for a level, keep looking. After clearing a level, you will return " \
               "to town where you can stock up and heal for your next journey down into the abyss.  Initially, " \
